...
void ClipPolys(RENDERLIST4DV2& rend_list, CAM4DV1& cam, int clip_flags)
{
		 ...
		 // test if any vertex has protruded beyond near clipping plane?
			if (((vertex_ccodes[0] | vertex_ccodes[1] | vertex_ccodes[2]) & CLIP_CODE_LZ))
			{
				...
				if (num_verts_in == 1)
				{
					...
					// clip edge v0.v1
					v.set(curr_poly->tvlist[v0].v, curr_poly->tvlist[v1].v);
					...
					// clip edge v0.v2
					v.set(curr_poly->tvlist[v0].v, curr_poly->tvlist[v2].v);
				   ...
				   // build u, v
					u.set(curr_poly->tvlist[v0].v, curr_poly->tvlist[v1].v);
					v.set(curr_poly->tvlist[v0].v, curr_poly->tvlist[v2].v);
					...
					curr_poly->nlength = LengthFast(n);
				...
				// step 0: copy the polygon
					...
					// build u, v
					u.set(temp_poly.tvlist[v0].v, temp_poly.tvlist[v1].v);
					v.set(temp_poly.tvlist[v0].v, temp_poly.tvlist[v2].v);
...
int Generate_Terrain_OBJECT4DV2(OBJECT4DV2& obj,     // pointer to object
...
// load texture map if there is one
	if ((poly_attr & POLY4DV2_ATTR_SHADE_MODE_TEXTURE) && texture_map_file)
	{
		...
		obj.texture = (BITMAP_IMAGE&)malloc(sizeof(BITMAP_IMAGE));
...
int LightWorld(OBJECT4DV2& obj,  // object to process
	CAM4DV1& cam,     // camera position
	LIGHTV2* lights,  // light list (might have more than one)
				...
				if (lights[curr_light].attr & LIGHTV2_ATTR_AMBIENT)
				{
				...
				else if (lights[curr_light].attr & LIGHTV2_ATTR_INFINITE) ///////////////////////////////////////////
					{
					 ...
					 //nl = VECTOR4D_Length_Fast2(&n);
						...
						dp = Dot(n, lights[curr_light].tdir);
					 ...
					 //ERROR_MSG("\ninfinite sum=%d,%d,%d", r_sum, g_sum, b_sum);
					...
					else if (lights[curr_light].attr & LIGHTV2_ATTR_POINT) ///////////////////////////////////////
						{
						 ...
						 //nl = VECTOR4D_Length_Fast2(&n);
							...
							l.set(obj.vlist_trans[vindex_0].v, lights[curr_light].tpos);
				...
				for (int curr_light = 0; curr_light < max_lights; curr_light++)
				{
					...
					if (lights[curr_light].attr & LIGHTV2_ATTR_AMBIENT) ///////////////////////////////
					{
						...
						if (lights[curr_light].attr & LIGHTV2_ATTR_INFINITE) /////////////////////////////////
						{
							...
							// vertex 0
							dp = Dot(obj.vlist_trans[vindex_0].n, lights[curr_light].tdir);
						 ...
						 // vertex 1
							dp = Dot(obj.vlist_trans[vindex_1].n, lights[curr_light].tdir);
						 ...
						 // vertex 2
							dp = Dot(obj.vlist_trans[vindex_2].n, lights[curr_light].tdir);
								 ...
								 //ERROR_MSG("\nexiting spotlight1, sums rgb0[%d, %d, %d], rgb1[%d,%d,%d], rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0,  r_sum1, g_sum1, b_sum1,  r_sum2, g_sum2, b_sum2);
									...
									if (lights[curr_light].attr & LIGHTV2_ATTR_SPOTLIGHT2) // simple version //////////////////////////
									{
									 ...
									 // vertex 1
										...
										{
											// compute vector from light to surface (different from l which IS the light dir)
											s.set(lights[curr_light].tpos, obj.vlist_trans[vindex_1].v);
									 ...
									 // vertex 2
										...
										{
											// compute vector from light to surface (different from l which IS the light dir)
											s.set(lights[curr_light].tpos, obj.vlist_trans[vindex_2].v);
...
int Light_RENDERLIST4DV2_World2_16(RENDERLIST4DV2& rend_list,  // list to process
	CAM4DV1& cam,     // camera position
	LIGHTV2* lights,  // light list (might have more than one)
	int max_lights)      // maximum lights in list
{
					 ...
					 //ERROR_MSG("\ninfinite sum=%d,%d,%d", r_sum, g_sum, b_sum);
						...
						if (lights[curr_light].attr & LIGHTV2_ATTR_POINT) ///////////////////////////////////////
						{
						 ...
						 //nl = VECTOR4D_Length_Fast2(&n);
							...
							// compute vector from surface to light
							l.set(curr_poly->tvlist[0].v, lights[curr_light].tpos);
							 ...
							 //ERROR_MSG("\nspotlight sum=%d,%d,%d",r_sum, g_sum, b_sum);
								...
								if (lights[curr_light].attr & LIGHTV2_ATTR_SPOTLIGHT2) // simple version ////////////////////
								{
								 ...
								 //nl = VECTOR4D_Length_Fast2(&n);
									...
									{
										// compute vector from light to surface (different from l which IS the light dir)
										s.set(lights[curr_light].tpos, curr_poly->tvlist[0].v);
				...
				for (int curr_light = 0; curr_light < max_lights; curr_light++)
				{
					...
					if (lights[curr_light].attr & LIGHTV2_ATTR_AMBIENT) ///////////////////////////////
					{
						...
						if (lights[curr_light].attr & LIGHTV2_ATTR_INFINITE) /////////////////////////////////
						{
							...
							// vertex 0
							dp = Dot(curr_poly->tvlist[0].n, lights[curr_light].tdir);
						 ...
						 // vertex 1
							dp = Dot(curr_poly->tvlist[1].n, lights[curr_light].tdir);
						 ...
						 // vertex 2
							dp = Dot(curr_poly->tvlist[2].n, lights[curr_light].tdir);
								 ...
								 //ERROR_MSG("\nexiting spotlight1, sums rgb0[%d, %d, %d], rgb1[%d,%d,%d], rgb2[%d,%d,%d]", r_sum0, g_sum0, b_sum0,  r_sum1, g_sum1, b_sum1,  r_sum2, g_sum2, b_sum2);
									...
									if (lights[curr_light].attr & LIGHTV2_ATTR_SPOTLIGHT2) // simple version //////////////////////////
									{
									 ...
									 // vertex 1
										...
										{
											// compute vector from light to surface (different from l which IS the light dir)
											s.set(lights[curr_light].tpos, curr_poly->tvlist[1].v);
									 ...
									 // vertex 2
										...
										{
											// compute vector from light to surface (different from l which IS the light dir)
											s.set(lights[curr_light].tpos, curr_poly->tvlist[2].v);
...
void Transform(LIGHTV2* lights,  // array of lights to transform
 ...
 // what coordinates should be transformed?
	...
	case TRANSFORM_LOCAL_ONLY:
	{
		...
		for (curr_light = 0; curr_light < num_lights; curr_light++)
		{
			...
			Mul(lights[curr_light].pos, mt, presult);

			// store result back
			lights[curr_light].pos = presult;

			// transform direction vector 
			Mul(lights[curr_light].dir, mr, presult);

			// store result back
			lights[curr_light].dir = presult;
		} // end for
	...
	case TRANSFORM_TRANS_ONLY:
	{
		...
		for (curr_light = 0; curr_light < num_lights; curr_light++)
		{
			...
			// transform point position of each light
			Mul(lights[curr_light].tpos, mt, presult);

			// store result back
			lights[curr_light].tpos=presult;

			// transform direction vector 
			Mul(lights[curr_light].tdir, mr, presult);

			// store result back
			lights[curr_light].tdir=presult;
		} // end for
	...
	case TRANSFORM_LOCAL_TO_TRANS:
	{
		...
		for (curr_light = 0; curr_light < num_lights; curr_light++)
		{
			...
			// transform point position of each light
			Mul(lights[curr_light].pos, mt, lights[curr_light].tpos);

			// transform direction vector 
			Mul(lights[curr_light].dir, mr, lights[curr_light].tdir);
		} 
